<script lang="ts">
  import { Rack } from "../model/state";
    ...
    type Dropdown = { title: string; dropdown: { title: string; action: () => void; }[]};
...
<nav class="nav">
    <ul class="nav nav-tabs">
        ...
        <div class="dropdown">
            <a class="nav-link dd-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                {replace(dropdown.title, { display })}
            </a>
            <ul class="dropdown-menu">
                {#each dropdown.dropdown as item, j}
                    <li><a class="dropdown-item" on:click={item.action}>{replace(item.title, { display, perform: performing ? 'Stop' : 'Start' })}</a></li>
                {/each}
            </ul>
          </div>
        {/each}
